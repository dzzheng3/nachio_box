...
public class DefaultStyleFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener, BaseQuickAdapter.RequestLoadMoreListener {
    ...
    private static final String JUHE_NEWS_APP_KEY = "9a2d175e3d944c3f9b0b2f25aaef7d5f";
...
//    private String mParam2;
    ...
    private List<NewsItem.ArticlesBean> mNewsItemList;
    ...
    private void setNewDataAddList(NewsItem newsItem) {
        Logger.e(newsItem.getStatus()+"--------------");
        if (newsItem != null && newsItem.getStatus().equals("ok")) {
            baseQuickAdapter.setNewData(newsItem.getArticles());
            if (mSwipeRefreshLayout.isRefreshing()) {
                mSwipeRefreshLayout.setRefreshing(false);
            }
            if (!mSwipeRefreshLayout.isEnabled()) {
    ...
    private void initRecyclerView() {
        ...
        mNewsList.addOnItemTouchListener(new OnItemClickListener() {
            ...
            public void onSimpleItemClick(BaseQuickAdapter adapter, View view, int position) {
                List<NewsItem.ArticlesBean> data = adapter.getData();
